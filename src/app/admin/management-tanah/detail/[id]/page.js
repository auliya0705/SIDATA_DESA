...
export default function DetailTanahOwnerPage() {
  ...
  const [tanah, setTanah] = useState(null); // response penuh /tanah/{id}
  const SHOW_ID_COL = false; 
  // ===== Helper: muat ulang detail tanah =====
  ...
  const ownerVM = useMemo(() => {
    ...
    return {
      ...
      total_luas: Number(tanah?.jumlah_m2_computed ?? tanah?.total_luas_m2 ?? 0),
  ...
  // ===== Actions =====
    ...
    if (!confirm("Yakin ingin mengajukan penghapusan bidang ini?")) return;

    try {
      // Safe resolver dengan fallback bila path di config undefined
      const delEndpoint =
        API_ENDPOINTS?.STAFF?.PROPOSALS?.BIDANG?.DELETE
          ? API_ENDPOINTS.STAFF.PROPOSALS.BIDANG.DELETE(bidangId)
          : `/staff/proposals/bidang/${bidangId}`;

      await apiDelete(delEndpoint);

      alert("Proposal hapus bidang dibuat. Menunggu persetujuan Kepala Desa.");
      await loadDetail(); // refetch tabel
    } catch (err) {
      console.error("Hapus bidang gagal:", err);
      alert(err?.message || "Gagal mengajukan hapus bidang.");
    }
  };
          ...
          <div>
            ...
            <div className="flex gap-6 text-sm text-gray-600">
              ...
              <div className="hidden md:block">
                ...
                <span className="text-gray-900">{tanah?.nomor_urut ?? "-"}</span>
              </div>
            </div>
          </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-teal-700 text-white">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Luas (m²)</th>
              <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Status Hak</th>
              <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Penggunaan</th>
              <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Keterangan</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {bidangList.map((b, idx) => (
              <tr key={b.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-700">{b.keterangan}</td>
                <td className="px-6 py-4 text-sm">
