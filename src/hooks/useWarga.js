...
export function useWarga() {
  ...
  /**
   * Delete warga via proposal (POST + _method=DELETE)
   */
  ...
  try {
    ...
    // pakai endpoint dari config
    const endpoint = API_ENDPOINTS.STAFF.PROPOSALS.WARGA.DELETE(id);
    const url = getApiUrl(endpoint);

    const res = await fetch(url, {
      method: "POST", // POST + _method=DELETE (sesuai Laravel)
      headers: {
  ...
  const checkNikUnique = useCallback(async (nik, ignoreId = null) => {
    ...
    try {
      ...
      const res1 = await apiGet(
        `${API_ENDPOINTS.WARGA.LIST}?nik=${encodeURIComponent(nik)}`
      );
      ...
      if (!list.length) {
        ...
        const res2 = await apiGet(
          `${API_ENDPOINTS.WARGA.LIST}?search=${encodeURIComponent(nik)}`
        );
        list = pickList(res2);
      ...
      // kalau semua gagal, jangan menghalangi proses
      return { exists: false };
    }
  }, []);
