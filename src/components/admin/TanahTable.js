      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-teal-700 text-white">
          <tr>
            ...
            <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Nama Pemilik</th>
            <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Jumlah Luas (m²)</th>
            <th className="px-6 py-4 text-left text-xs font-semibold uppercase">Jumlah Bidang</th>
          ...
          {tanahData.map((tanah, index) => (
            <tr key={tanah.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 text-sm text-gray-700 relative">
                <button
                  onClick={() =>
                    setShowActionMenu(showActionMenu === tanah.id ? null : tanah.id)
                  }
                ...
                {showActionMenu === tanah.id && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    ...
                    <button
                      onClick={() => {
                        if (
                          confirm("Yakin ingin menghapus semua data tanah pemilik ini?")
                        ) {
                    ...
                    >
                      <Trash2 size={16} />
                      <span>{deletingId === tanah.id ? "Menghapus…" : "Hapus Semua"}</span>
                    </button>
                  </div>
                )}
              </td>
