...
function getApprovalId(row) {
  return row?.approval_id ?? row?.request_id ?? row?.proposal_id ?? row?.id;
}
...
export default function ApprovalTable({
  ...
  // Dialog state for error details
            ...
            {rows.map((row, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-gray-800">{nama}</td>
                  ...
                  <td className="px-4 py-3 text-sm">
                    ...
                    {row.apply_error && (
                      <button
                        onClick={() => handleShowError(row.apply_error)}
                  ...
                  <td className="px-4 py-3 text-sm">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          onViewDetail?.({ ...row, approval_id: approvalId })
                        }
                      ...
                      <button
                        onClick={() =>
                          isPending &&
                          onApprove?.({ ...row, approval_id: approvalId })
                        }
                        ...
                        title={
                          isPending
                            ? "Approve"
                            : "Hanya bisa approve saat status Pending"
                        }
                      ...
                      <button
                        onClick={() =>
                          isPending &&
                          onReject?.({ ...row, approval_id: approvalId })
                        }
                        ...
                        title={
                          isPending
                            ? "Tolak"
                            : "Hanya bisa menolak saat status Pending"
                        }
                        disabled={!isPending || !approvalId}
                      >
                        <X size={16} />
                    ...
                    {!approvalId && (
                      <div className="mt-2 text-[11px] text-amber-700">
                        ⚠️ Tidak menemukan approval_id pada baris ini — tombol
                        dinonaktifkan.
                      </div>
                    )}
                  </td>
